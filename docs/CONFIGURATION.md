...
| Max stake per member, per side | — (DB column `selections.max_stake_cents`) | none | The same cap scoped to one outcome, so the longshot can be capped at $50 while the short price stays open. Counts that member's pending and accepted stake on that selection only. Both caps apply when both are set. | Per selection on the market create form, or on the Markets page while the market is draft or open |
