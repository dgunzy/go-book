...
func (dao *UserDAO) GetUserBetsByBetID(betID int) ([]*models.UserBet, error) {
	var userBets []*models.UserBet
	query := `SELECT Amount, PlacedAt, Result, BetDescription, Odds FROM UserBets WHERE UserBetID = ?`
	...
	for rows.Next() {
		var ub models.UserBet
		err := rows.Scan(&ub.Amount, &ub.PlacedAt, &ub.Result, &ub.BetDescription, &ub.Odds)
		if err != nil {
			fmt.Println(err)
			return nil, err
		}
...
func (dao *UserDAO) GetAllUserBets() ([]*models.UserBet, error) {
	...
	query := `SELECT Amount, PlacedAt, Result, BetDescription, Odds,UserID, Approved FROM UserBets`
	...
	for rows.Next() {
		var ub models.UserBet
		err := rows.Scan(&ub.Amount, &ub.PlacedAt, &ub.Result, &ub.BetDescription, &ub.Odds, &ub.UserID, &ub.Approved)
		if err != nil {
